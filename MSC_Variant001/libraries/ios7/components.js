	...
	,afterDisplay: function(item,containerid,symbol) {
		...
		if(!prx.editor) {
            ...
            if(_isCheckboxAndRadioGrouped) {
                $('label[for="'+_id+'-checkbox"]').on('mousedown', function(event) {
                ...
                $('label[for="'+_id+'-checkbox"]').on('click', function(event) {
                    event.preventDefault();
                });
...
prx.types.ios8_searchbar = {
	...
	,onDisplay: function(item,containerid,symbol) {
		...
		if(prx.editor) {
			...
			cR += '<div class="ios8-searchbar-faux-input-overlay liveUpdate-backgroundColor">'+(prx.componentsHelper.getProp(item.imgSrc.fileId,'other') != '' ? '<span class="ios8-searchbar-icon"></span>': '')+'<span class="liveUpdate-placeholderColor placeholder-input">'+prx.componentsHelper.getProp(item.placeholder,'other')+'</span></div>';
