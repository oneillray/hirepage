	...
	,onDisplay: function(item,containerid,symbol) {
		...
		if (_symbolindex > -1) {
            ...
            if (prx.editor && symbol === undefined) {
                cReturn += '<div class="type-image-hover-message">Double-click to edit container</div>';
            }
...
prx.types.placeholder = {
	...
	,onDisplay: function(item,containerid,symbol) {
		...
		cR += '<div class="diagonal diagonal1 liveUpdate-borderColor changeProperty-borderColor" style="border-color: '+prx.componentsHelper.getProp(item.borderColor,'color-border')+'; width: '+_width+'px; -moz-transform: rotate('+_angle1+'deg); -webkit-transform: rotate('+_angle1+'deg); -o-transform: rotate('+_angle1+'deg); transform: rotate('+_angle1+'deg);  border-top-width: '+prx.componentsHelper.getProp(item.thickness,'num-border-width')+'px; left: '+prx.componentsHelper.getProp(item.thickness,'num-other')/2+'px;"></div>'
		cR += '<div class="diagonal diagonal2 liveUpdate-borderColor changeProperty-borderColor" style="border-color: '+prx.componentsHelper.getProp(item.borderColor,'color-border')+'; width: '+_width+'px; -moz-transform: rotate(-'+_angle1+'deg); -webkit-transform: rotate(-'+_angle1+'deg); -o-transform: rotate(-'+_angle1+'deg); transform: rotate(-'+_angle1+'deg); border-top-width: '+prx.componentsHelper.getProp(item.thickness,'num-border-width')+'px; right: '+prx.componentsHelper.getProp(item.thickness,'num-other')/2+'px;"></div>'
		//if(item.text != "") {
		...
		$.each(item.tabs, function(i,elm){
			...
			if(prx.componentsHelper.getProp(item.iconpos,'other') != "") {
				...
				if(prx.componentsHelper.getProp(item.maskEnabled,'boolean') && $.browser.webkit) {
					...
					cR += '#'+_id+' label[for='+_id+'-radio-'+i+'] .icon, #'+_id+' label[for='+_id+'-radio-'+i+'-overlay] .icon { -webkit-mask-image: url("'+assetPathWithMaskEnabled+'"); }'
				} else {
